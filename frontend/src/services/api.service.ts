...
export const uploadService = {
  async uploadFile(file: File): Promise<Upload> {
    ...
    const res = await api.post<Upload>('/uploads', form, {
...
export const analysisService = {
  async startAnalysis(uploadId: string): Promise<AnalysisStatus> {
    const res = await api.post<AnalysisStatus>(`/analysis/${uploadId}`)
    return res.data
  },

  async getAnalysis(uploadId: string): Promise<AnalysisDetail> {
    const res = await api.get<AnalysisDetail>(`/analysis/${uploadId}`)
    return res.data
  },
}
