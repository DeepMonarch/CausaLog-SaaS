...
export default function AnalysisPage() {
  ...
  useEffect(() => {
    if (analysis?.status === 'completed' || analysis?.status === 'failed') {
      // polling will be cleared on next render cycle
    }
  }, [analysis?.status])
    ...
    <div className="p-8 max-w-5xl mx-auto animate-fade-in">
      ...
      {analysis && (
        <div className="space-y-6">
          {/* Status bar */}
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-bg-border" style={{ background: '#0f1526' }}>
            <StatusDot status={analysis.status} />
            <span className="font-mono text-sm text-text-secondary capitalize">{analysis.status}</span>
            {analysis.status === 'running' && (
              <span className="font-mono text-xs text-text-muted">— pipeline running, auto-refreshing…</span>
            )}
            {analysis.status === 'failed' && analysis.error_message && (
              <span className="font-mono text-xs text-severity-high ml-2">{analysis.error_message}</span>
            )}
            {analysis.status === 'completed' && (
              <Link to="/reports" className="ml-auto font-mono text-xs text-accent-cyan hover:text-white transition-colors">
                View full report →
              </Link>
            )}
          </div>
            ...
            <div className="rounded-xl border border-bg-border p-6" style={{ background: '#0f1526' }}>
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  ...
                  <p className="font-mono text-xs text-text-muted mt-1">Rule: {inference.rule_id}</p>
                </div>
              ...
              {aiExplanation && (
                <div className="mb-5 p-4 rounded-lg border border-accent-purple border-opacity-20"
                  style={{ background: 'rgba(167,139,250,0.04)' }}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-accent-purple text-xs">✦</span>
                    <p className="font-mono text-xs text-accent-purple uppercase tracking-widest">AI Summary</p>
                    <span className="ml-auto font-mono text-xs text-text-muted">{aiExplanation.model_used}</span>
                  </div>
                  <p className="text-text-secondary text-sm leading-relaxed mb-3">{aiExplanation.ai_summary}</p>
                  <p className="font-mono text-xs text-text-muted italic">{aiExplanation.ai_confidence_note}</p>
                </div>
              )}

              <div className="mb-4">
                <p className="font-mono text-xs text-text-muted mb-2">Technical explanation</p>
            ...
            <div className="rounded-xl border border-bg-border p-5" style={{ background: '#0f1526' }}>
              ...
              <div className="flex flex-wrap gap-2">
                {keywords.length > 0 ? keywords.map(kw => (
                  <span key={kw} className="px-2 py-0.5 rounded font-mono text-xs text-accent-cyan"
                    style={{ background: 'rgba(0,212,255,0.08)' }}>
                    {kw}
                  </span>
                )) : <span className="text-text-muted text-xs font-mono">None extracted</span>}
              </div>
            </div>
              ...
              {anomalies ? (
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono text-xs text-severity-high font-semibold">{anomalies.total_anomalies}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {anomalies.anomalous_records.slice(0, 3).map((r, i) => (
                    <div key={i} className="mt-2 p-2 rounded bg-bg-elevated border border-bg-border">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-mono text-xs text-text-muted ml-auto">score {r.anomaly_score.toFixed(3)}</span>
                      </div>
          ...
          {clusters.length > 0 && (
            <div className="rounded-xl border border-bg-border p-5" style={{ background: '#0f1526' }}>
              ...
              <div className="grid gap-3">
                {clusters.map(c => (
                  <div key={c.cluster_id} className="p-4 rounded-lg border border-bg-border bg-bg-elevated">
                    ...
                    <div className="space-y-1">
                      {c.sample_messages.slice(0, 2).map((msg, i) => (
                        <p key={i} className="font-mono text-xs text-text-muted truncate">· {msg}</p>
                      ))}
                    </div>
                  </div>
                ))}
      ...
      {!analysis && !loading && !error && (
        <div className="text-center py-16">
          <p className="font-mono text-sm text-text-muted mb-3">
            Upload a log file from the Dashboard to see analysis results here.
          </p>
